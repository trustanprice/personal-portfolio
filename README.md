...
- Displays my key technical projects such as:
  ...
  - **LedgerOne** — a single-entry, append-only fintech ledger (Python + dbt + DuckDB, staging → star schema → tested reporting marts) with a companion credit-risk module (vintage curves, roll-rate transition matrix, CECL reserve estimate) validated within 0.19pp of real Freddie Mac mortgage outcomes, genuinely deployed to AWS via CloudFormation and OIDC-secured CI/CD to MotherDuck (136/136 dbt tests passing in the cloud), and presented through a multi-tab [live site](https://ledgerone-green.vercel.app/) — teaching walkthrough, data-engineering reference, forecasting dashboard, infrastructure writeup, and an emerging-techniques roadmap.  
